...
func TestLocalConfig(t *testing.T) {
	...
	for key, value := range expectedFeatureConfig {
		exp, ok := actualFeatureConfig[key]
		assert.True(t, ok)

		slice, ok := exp.([]domain.FeatureFlag)
		if ok {
			assert.Equal(t, value, slice)
			continue
		}

		ptr, ok := exp.(*domain.FeatureFlag)
		if ok {
			act, ok := value.(*domain.FeatureFlag)
			assert.True(t, ok)

			assert.Equal(t, *ptr, *act)
		}
	}

	for key, value := range expectedTargetConfig {
		exp, ok := actualTargetConfig[key]
		assert.True(t, ok)

		slice, ok := exp.([]domain.Target)
		if ok {
			assert.Equal(t, value, slice)
			continue
		}

		ptr, ok := exp.(*domain.Target)
		if ok {
			act, ok := value.(*domain.Target)
			assert.True(t, ok)
	...
	for key, value := range expectedSegments {
		exp, ok := actualSegments[key]
		assert.True(t, ok)

		slice, ok := exp.([]domain.Segment)
		if ok {
			assert.Equal(t, value, slice)
			continue
		}

		ptr, ok := exp.(*domain.Segment)
		if ok {
			act, ok := value.(*domain.Segment)
			assert.True(t, ok)
